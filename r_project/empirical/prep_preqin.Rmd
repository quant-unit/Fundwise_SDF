...
process_plot_data <- function(df, weight_label) {
    df.pe <- df[df$type == "PE", ]

    df.pe$Date <- as.Date(df.pe$Date)
    df.pe$Year <- as.numeric(format(df.pe$Date, "%Y"))
    df.pe$Age <- df.pe$Year - as.numeric(as.character(df.pe$Vintage))

    # Filter up to year 20 (t+20)
    df.pe <- df.pe[df.pe$Age >= 0 & df.pe$Age <= 20, ]

    agg <- aggregate(cbind(CF, CON) ~ Vintage + Age, data = df.pe, sum, na.rm = TRUE)
...
agg.fw <- process_plot_data(df.out.fw, "Fund-size weighted")
agg.ew <- process_plot_data(df.out.ew, "Equal weighted")
...
p_jcurve <- ggplot(agg.all, aes(x = Age, y = CumCF_Pct, color = Vintage)) +
    ...
    labs(
        title = "Cumulative net cash flows since inception (all available PE vintages in Preqin dataset)",
        y = "",
        x = "",
        color = "Vintage"
    )
...
ggsave("cumcashflowsPE.pdf", p_jcurve, width = 10, height = 5)
```
