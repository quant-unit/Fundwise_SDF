...
make.vintage.summary <- function(asset.class = "Private Equity", region = NA) {
    ...
    if (!is.na(region)) {
        ...
        if (region == "North America") {
            ...
            df <- df[df$PRIMARY.GEOGRAPHIC.FOCUS %in% c("US", "US, West Europe", "North America",
                                                        "Northeast", "Southwest", "Southeast", "West", "Midwest"), ]
        }
    }
...
df.out.fw <- make.preqin.csv(TRUE, TRUE, "North America")
df.out.ew <- make.preqin.csv(FALSE, TRUE, "North America")
...
df.out <- make.preqin.csv(TRUE, TRUE, "North America", 0.25) # 50% final NAV discount
df.out <- make.preqin.csv(FALSE, TRUE, "North America", 0.25) # 50% final NAV discount
df.out <- make.preqin.csv(TRUE, TRUE, "North America", 0.75) # 50% final NAV discount
df.out <- make.preqin.csv(FALSE, TRUE, "North America", 0.75) # 50% final NAV discount
df.out <- make.preqin.csv(TRUE, TRUE, "North America", 0) # 50% final NAV discount
df.out <- make.preqin.csv(FALSE, TRUE, "North America", 0) # 50% final NAV discount
...
```{r plot_cum_cf, fig.width=10, fig.height=5, warning=FALSE, message=FALSE}
library(ggplot2)
library(dplyr)
...
for (f_type in c("PE", "BO", "VC")) {
    ...
    vintages <- levels(agg.all$Vintage)

    target_vintages <- as.character(2011:2021)
    target_vintages <- target_vintages[target_vintages %in% vintages]
    other_vintages <- setdiff(vintages, target_vintages)

    # Full plasma scale for older vintages
    other_colors <- scales::viridis_pal(option = "plasma", end = 0.9)(length(other_vintages))
    names(other_colors) <- other_vintages

    # Distinct teal/green gradient for 2011-2021 so they stand out but don't merge into a single solid color
    target_colors <- grDevices::colorRampPalette(c("#66c2a4", "#006d2c"))(length(target_vintages))
    names(target_colors) <- target_vintages

    vintage_colors <- c(other_colors, target_colors)
