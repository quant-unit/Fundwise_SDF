...
required_q5 <- c("year", "month", "R_MKT")
if (!all(required_q5 %in% names(q5_raw))) {
  stop("q5 factor file does not contain required columns year, month, R_MKT.")
}
...
q5_quarterly <- q5_raw %>%
  ...
  group_by(qtr_end) %>%
  summarise(mkt_ret_q = prod(1 + mkt_ret_m, na.rm = TRUE) - 1, .groups = "drop")
...
beta_plot_df <- dimson_summary %>%
  ...
  mutate(method = recode(method, beta_naive = "Naive OLS (lag 0)", beta_dimson = "Dimson sum(beta_0...beta_L)"))
...
p_beta <- ggplot(beta_plot_df, aes(x = series_label, y = beta, fill = method)) +
  ...
  labs(
    title = "Naive vs Dimson Beta to q5 Market Factor",
    subtitle = "Series labels include Dimson lag length L",
    x = NULL,
    y = "Estimated beta",
    fill = NULL
  ) +
