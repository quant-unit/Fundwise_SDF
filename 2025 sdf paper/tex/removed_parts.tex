...

{
	...
	Mathematically, we can derive this kernel by projecting the true SDF onto the space spanned by the realized public market factors \citep{HJ91}. 
	...
	\paragraph{Connection to \cite{KN16}}
	...
	\paragraph{``Explaining away" the Timing Risk Premium}
	...
	Let us define the \emph{NFV Error} moment condition for estimation as:
	\begin{equation}
		m^{\mathrm{NFV}}_{i,j}(\theta) = \frac{\Psi_{t_{i,j}^{\mathrm{Future}}}(\theta)}{\Psi_{\tau}(\theta)} \delta_{i,j}(\theta)
	\end{equation}
	...
	\begin{equation}
		\mathbb{E} \left[ m^{\mathrm{NFV}}_{i,j}(\theta) \mid \mathcal{F}_{\tau} \right] = a_{i,j}^{\mathrm{True}}(\theta) + \text{Timing Risk Premium}_{i,j}(\theta)
	\end{equation}
	...
	Consequently, an estimator minimizing the objective function $\mathcal{Q}^{\mathrm{NFV}}(\theta) = \left( \frac{1}{n} \sum m^{\mathrm{NFV}}_{i,j}(\theta) \right)^2$ will maintain the equality:
	\begin{equation}
	...
	Thus, the NFV formulation generates \textbf{unrelated bias terms} that inevitably confound the identification of the true pricing kernel, rendering it unsuitable for ex-ante estimation.
	...
	\begin{tcolorbox}[colback=gray!10, colframe=gray!50, title=Outlook: Simulation results using realized factor returns]
		Our simulation results in Section \ref{sec:simulation_study} show a relatively large alpha term when we discount to the fund inception date (as the NPV theory suggests). Adding more ``biased" NFV dates (to the optimal NPV date) in the target function, we want to minimize, reduces this alpha term more and more.
		This could indicate (as a currently unverified conjecture) that the NFV approach could be beneficial under certain circumstances when we want to condition on realized factor returns (e.g., for benchmarking purposes).
