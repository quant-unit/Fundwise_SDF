...
\begin{align}
	...
	&= 
	\underbrace{\mathbb{E} \left[ \frac{\Psi_{t_{i,j}^{\mathrm{Inv}}}}{\Psi_{\tau}} \frac{a_{i,j}^{\text{True}}}{R_{f, t_{i,j}^{\mathrm{Inv}} \to t_{i,j}^{\mathrm{Future}}}} \mid \mathcal{F}_{\tau} \right]}_{\text{Skill}}
	\\ \nonumber
	&+ 
...
Even if a manager has no skill ($a^{\text{True}} = 0$), the measured alpha will include the timing risk premium. 
...
Estimating the parameters $\hat{\theta}$ on the full sample of public returns is equivalent to finding the element of the parametric family $\{\Psi_t(\theta) : \theta \in \Theta\}$ closest to $\Psi_t^{\mathrm{true}}$ in the $L^2$ sense, i.e., the orthogonal projection onto the linear span of functions of the public factor history $F_0, \ldots, F_T$:
\begin{equation}
	\Psi_t^*(\hat{\theta}) = \arg\min_{\theta \in \Theta} \left\| \Psi_t(\theta) - \Psi_t^{\mathrm{true}} \right\|_{L^2}.
\end{equation}
...
\begin{equation}
	a_{i} = \sum_{t=0}^T \frac{\Psi_t}{\Psi_{0}} CF_{i,t}
\end{equation}
...
Consequently, a non-zero $a_i$ captures the component of performance that is orthogonal to the realized public opportunity set (True Skill), effectively filtering out the systematic timing gains (Macro Luck) that are spanned by $\Psi_t / \Psi_{0}$.
...
Furthermore, while \cite{KN16} focus on valuing the entire fund stream at inception ($\tau = 0$), our Case C analysis extends this logic to future compounding dates, confirming that the same kernel properly risk-adjusts for realized timing premiums in ex-post performance attribution (cf.\ Equation~\ref{eq:future_value_bias_derivation}).
...
\begin{equation}
	\mathbb{E} \left[ m^{\mathrm{FPV}}_{i,j}(\theta) \mid \mathcal{F}_{\tau} \right] = a_{i,j}^{\mathrm{True}}(\theta) + \text{Timing Risk Premium}_{i,j}(\theta)
\end{equation}
Assuming the true model holds ($a^{\mathrm{True}}=0$), the moment condition does not equal zero but rather the \emph{systematic} Timing Risk Premium. 
Crucially, this premium does not vanish in aggregation; it remains positive for any asset with systematic risk exposure.
Consequently, an estimator minimizing the objective function $\mathcal{Q}^{\mathrm{FPV}}(\theta) = \left( \frac{1}{n} \sum m^{\mathrm{FPV}}_{i,j}(\theta) \right)^2$ will maintain the equality:
\begin{equation}
	\hat{a}_{\mathrm{est}} + \widehat{\text{Premium}} \approx 0 \implies \hat{a}_{\mathrm{est}} \approx - \widehat{\text{Premium}}
\end{equation}
