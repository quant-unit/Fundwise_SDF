...
In a nutshell, the \cite{DLP12} estimator determines the optimal alpha and beta parameters of a linear factor model by minimizing the squared net present value (NPV) of all $N$ fund cash flows in a least-squares optimization problem.
\begin{equation}
...
\]
Here, the \cite{DLP12} approach implicitly assumes that the $NPV$ can be perceived as ``pricing error" with a zero expected value.
...
Then the term $\mathbb{E} \left[ \delta_{i,j} \mid \mathcal{F}_{\tau} \right]$ is the realized pricing error up to time $\tau$ plus the expected future pricing error. It decomposes as:
...
For \emph{ex-ante} asset pricing tests, the Net Future Value approach is misspecified: the non-zero expected covariance implies that standard moment conditions $\mathbb{E}[Error]=0$ are violated, leading to biased parameter estimates if not explicitly corrected for this risk premium.
...
Specifically, we can mathematically decompose the expected FPV pricing error into two distinct components:
\begin{align}
	...
	&= 
	\underbrace{\mathbb{E} \left[ \frac{\Psi_{t_{i,j}^{\mathrm{Inv}}}}{\Psi_{\tau}} \frac{\alpha_{i,j}^{\text{True}}}{R_{f, t_{i,j}^{\mathrm{Inv}} \to t_{i,j}^{\mathrm{Future}}}} \mid \mathcal{F}_{\tau} \right]}_{\text{Skill}}
	\\ \nonumber
	&+ 
...
\end{align}
where $R_f$ is the gross risk-free return over the holding period and $\alpha^{\text{True}}$ is the deal's expected pricing error at inception.
Even if a manager has no skill ($\alpha^{\text{True}} = 0$), the measured alpha will include the timing risk premium. 
This is not a flaw but a feature of ex-post benchmarking: it correctly reflects that the investor was exposed to this specific risk path (path of SDF realizations).
However, for ex-ante pricing tests, ignoring this term leads to biased estimates of the SDF parameters.

\paragraph{SDF as Macro-Conditioning Kernel}

Current research often views the SDF solely as an unconditional pricing mechanism, searching for factors that set the average pricing error to zero across all possible states. 
Our decomposition in Equation \ref{eq:alpha_decomp} suggests a shift in perspective. 
When conditioned on a specific realized path of history (e.g., the stock market trajectory from 1990--2010), the SDF acts as a \emph{Macro-Conditioning Kernel}. 
In this ex-post view, the SDF does not just price risk; it filters out the ``macro luck", i.e., the systematic timing risk premium that any passive investor would have earned from that specific economic sequence.
Consequently, evaluating funds against this \emph{realized benchmarking kernel} allows us to distill the manager's \emph{True Alpha} conditional on the realized macro environment. 
Unlike a standard unconditional alpha, which conflates skill with the random realization of favorable economic regimes, this \emph{conditional alpha} specifically measures value add beyond the realized opportunity set of the economy.

Mathematically, we can derive this kernel by projecting the true SDF onto the space spanned by the realized public market factors. 
Let $\Psi_t^{\mathrm{true}}$ be the true, unobservable pricing kernel that satisfies $\mathbb{E}_t[\Psi_{t+1}^{\mathrm{true}} R_{i,t+1}] = 1$ for all assets. 
We approximate this kernel using a parametric function $\Psi_t(\theta)$ of public factors $F_t$ (e.g., a linear factor model). 
Estimating the parameters $\hat{\theta}$ on the full sample of public returns is equivalent to finding the orthogonal projection of $\Psi_t^{\mathrm{true}}$ onto the filtration $\mathcal{F}_T^F$ generated by the public factor history:
\begin{equation}
	\Psi_t^*(\hat{\theta}) = \mathrm{Proj} \left( \Psi_t^{\mathrm{true}} \mid \mathcal{F}_T^F \right).
\end{equation}
This projection $\Psi_t^*$ constitutes our \emph{Macro-Conditioning Kernel}. 
It represents the price of risk conditional on the \emph{ex-post} realization of the economy's path. 
When we price private equity cash flows using this kernel:
\begin{equation}
	\alpha_{i} = \sum_{t=0}^T \frac{\Psi_t}{\Psi_{0}} CF_{i,t}
\end{equation}
we are explicitly asking: ``What is the value of this fund's cash flows given the specific sequence of market shocks $F_0, \dots, F_T$ attempting to replicate them?'' 
Consequently, a non-zero $\alpha_i$ captures the component of performance that is orthogonal to the realized public opportunity set (True Skill), effectively filtering out the systematic timing gains (Macro Luck) that are spanned by $\Psi_t / \Psi_{0}$.

This interpretation of the SDF as a \emph{Macro-Conditioning Kernel} aligns conceptually with the Generalized Public Market Equivalent (GPME) framework introduced by \cite{KN16,KN22}. 
While Korteweg and Nagel do not explicitly use the term `kernel projection', their methodology fundamentally relies on the same principle: they construct a valuation benchmark by projecting private cash flows onto a \emph{realized} public market SDF. 
Specifically, their GPME metric evaluates the ``cost of replicating the fund's cash flows using public securities'' \citep{KN16}, effectively identifying the component of performance that could have been achieved by a passive investor facing the same sequence of market shocks. 
Our derivation clarifies the theoretical mechanism behind their approach: by defining the SDF $\Psi_t^*$ as the projection of the true kernel onto the realized filtration $\mathcal{F}_T^F$, we demonstrate \emph{why} their method correctly filters out systematic timing gains. 
Furthermore, while \cite{KN16} focus on valuing the entire fund stream at inception ($\tau = 0$), our Case C analysis extends this logic to future compounding dates, confirming that the same kernel properly risk-adjusts for realized timing premiums in ex-post performance attribution.
Thus, our Macro-Conditioning Kernel provides the rigorous asset pricing micro-foundation for the GPME's effectiveness as an ex-post benchmark.

\paragraph{``Explaining away" the Compounded Timing Risk Premium}

Finally, we can rigorously evaluate whether the ``Net Future Value" formulation in Equation \ref{eq:future_value_bias_derivation} can aid in empirically deriving the Macro-Conditioning Kernel ($\hat{\theta}$). 
Let us define the \emph{FPV Error} moment condition for estimation as:
\begin{equation}
	m^{\mathrm{FPV}}_{i,j}(\theta) = \frac{\Psi_{t_{i,j}^{\mathrm{Future}}}(\theta)}{\Psi_{\tau}(\theta)} \delta_{i,j}(\theta)
\end{equation}
A valid Generalized Method of Moments (GMM) estimator seeks parameters $\hat{\theta}$ that minimize the sample average of moment conditions which are theoretically zero in expectation.
However, taking the expectation conditional on the information set $\mathcal{F}_\tau$ at the reference date $\tau (< t_{i,j}^{\mathrm{Inv}} < t_{i,j}^{\mathrm{Future}})$ yields:
\begin{equation}
	\mathbb{E} \left[ m^{\mathrm{FPV}}_{i,j}(\theta) \mid \mathcal{F}_{\tau} \right] = \alpha_{i,j}^{\mathrm{True}}(\theta) + \text{Timing Risk Premium}_{i,j}(\theta)
\end{equation}
Assuming the true model holds ($\alpha^{\mathrm{True}}=0$), the moment condition does not equal zero but rather the \emph{systematic} Timing Risk Premium. 
Crucially, this premium does not vanish in aggregation; it remains positive for any asset with systematic risk exposure.
Consequently, an estimator minimizing the objective function $J(\theta) = \| \frac{1}{N} \sum m^{\mathrm{FPV}}_{i,j}(\theta) \|^2$ will maintain the equality:
\begin{equation}
	\hat{\alpha}_{\mathrm{est}} + \widehat{\text{Premium}} \approx 0 \implies \hat{\alpha}_{\mathrm{est}} \approx - \widehat{\text{Premium}}
\end{equation}
To satisfy the minimization condition, the estimator is forced to ``explain away'' the non-zero systematic risk premium by biasing the alpha estimate downwards. 
Thus, the FPV formulation generates \textbf{unrelated bias terms} that inevitably confound the identification of the true pricing kernel, rendering it unsuitable for ex-ante estimation.



% This box can be igonred and will be deleted in the final version!
\begin{tcolorbox}[colback=gray!10, colframe=gray!50, title=Outlook: Simulation results using realized factor returns]
	Our simulation results in Section \ref{sec:simulation_study} show a relatively large alpha term when we discount to the fund inception date (as the NPV theory suggests). Adding more ``biased" FPV dates (to the optimal NPV date) in the target function, we want to minimize, reduces this alpha term more and more.
	This could indicate (as a currently unverified conjecture) that the FPV approach could be beneficial under certain circumstances when we want to condition on realized factor returns (e.g., for benchmarking purposes).
	\textcolor{orange}{This will be further investigated in the simulation section!}
\end{tcolorbox}
...
Only for the fund's first investment date $\tau = \min_j t_{i,j}^{\mathrm{Inv}}$, we can show that the expected fund pricing error is equal to zero.
...
\begin{assume}
	\label{as:portfolio}
	For each vintage year, we pool fund cash flows to form $n_v$ 
	portfolios. Thus, $n = \sum_{v=1}^V n_v$. The boundary cases 
	are (i) single-fund portfolios and (ii) one portfolio per 
	vintage year.
\end{assume}
...
The pricing error $\bar{\epsilon}{i}$ is naturally indexed by both fund identity and vintage year, yielding a random field $\bar{\epsilon}{i} \equiv \bar{\epsilon}_{i,v_i}$ that fits neither standard time-series, cross-sectional, nor panel data frameworks (cf.\ Figure \ref{fig:randomfields}). 
...
A sufficient primitive condition for this ULLN is that $\bar{\epsilon}$ be spatial near-epoch dependent on an $\alpha$-mixing random field indexed by vintage years \citep{JP09,JP12}, meaning the dependence between funds decays sufficiently fast with vintage year distance. 
...
Since nominal fund sizes tend to grow over time, unweighted pricing errors $\bar{\epsilon}_i$ are non-stationary, which could violate the time trend part (i).
The weighting factor $w_i$ from Equation \ref{eq:average_pricing_error} (e.g., scaled by invested capital) can be used to normalize the pricing errors to achieve approximate stationarity, a prerequisite for the ULLN.


\subsubsection{Consistency}

The estimator $\hat{\theta}$ shall converge in probability to the true 
parameter value $\theta_0$ as the number of distinct vintage years goes 
to infinity.

\begin{proposition}[Consistency]
	\label{prop:consistency}
	Under Assumptions \ref{as:portfolio}--\ref{as:lln}, 
...
\begin{proof}
	...
	\begin{enumerate}
		...
		Since $L \geq 0$, we have $Q_0(\theta) \leq 0$ for all 
		$\theta$. For the case $L(x) = x^2$,
		\[
		Q_0(\theta) 
		= -\mathrm{Var}\bigl[\bar{\epsilon}(\theta)\bigr] 
		- \bigl(\mathbb{E}[\bar{\epsilon}(\theta)]\bigr)^2.
		\]
		Under Assumption~\ref{ass:unconditional_unbiased}, 
		Proposition~\ref{theo:inception_value} yields 
		$\mathbb{E}[\bar{\epsilon}(\theta_0)] = 0$, so the bias 
		term vanishes at $\theta_0$. For any $\theta \neq \theta_0$, 
		at least one of (a) the bias term 
		$(\mathbb{E}[\bar{\epsilon}(\theta)])^2 > 0$ or (b) the 
		variance term $\mathrm{Var}[\bar{\epsilon}(\theta)] > 
		\mathrm{Var}[\bar{\epsilon}(\theta_0)]$ must hold, ensuring 
		$Q_0(\theta) < Q_0(\theta_0)$. This is the standard 
		identification condition requiring that the true parameter 
		is the unique minimizer of the expected loss.
		
		\item \textit{Compactness.}
		The parameter space $\Theta$ is compact by economic 
		reasoning: factor loadings and intercepts are bounded 
		by plausible ranges implied by the risk--return tradeoff.
		
		\item \textit{Continuity.}
		Continuity of $Q_0(\theta)$ follows from the continuous 
		dependence of $\Psi_{\tau,t}(\theta)$ on $\theta$ and the 
		dominated convergence theorem, which is a standard 
		regularity condition.
		
		\item \textit{Uniform convergence.}
		By Assumption~\ref{as:lln}, 
		$\sup_{\theta \in \Theta} |Q_n(\theta) - Q_0(\theta)| 
		\xrightarrow{p} 0$, which directly satisfies the ULLN 
		requirement of \cite[Section~2.1]{NM94}.
	\end{enumerate}
\end{proof}
...
From Proposition \ref{prop:clt}, the asymptotic covariance matrix takes the sandwich form $\Sigma = H^{-1}\Lambda(H^{-1})^\top$ with $H$ and $\Lambda$ as defined therein. 
...
A key methodological difference is that \cite{KN16} estimate the SDF from public market 
data using a time-series GMM estimator and then apply the resulting model to evaluate PE cash flows, whereas we estimate the SDF directly from PE fund cash flows.
...
Note that since $L(x) = x^\top W x \geq 0$, maximizing $-L(\cdot)$ is equivalent to minimizing $L(\cdot)$; we omit the negative sign in Equation \ref{eq:kn16_estimator} for notational simplicity.
...
This means we condition our simulation analysis on realized $q$-investment factors paths (see the Marco-Conditioning Kernel discussion in Section \ref{sec:asset_pricing_pooled}).
