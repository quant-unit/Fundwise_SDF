...
\usepackage{graphics}
% use smaller margins
\usepackage[margin=1.25in]{geometry} % 1.25in
% use double spacing
\usepackage{setspace}
\usepackage{amsthm}
\usepackage{url}
...
This also holds when we limit the maximal fund lifetime to 10 years by reducing the maximum deal holding period from ten to five years. 
...
Especially, the variance of $\alpha$ estimates seem to be too high to allow reliable abnormal performance conclusions.
...
In the data-sparse private equity domain with only 20-40 cross-sectional units (i.e., vintage year portfolios) available for estimation, asymptotic inference seems not to be overly useful.
